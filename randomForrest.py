...
df_grid = pd.read_csv('parsed_data.csv')
all_records = []

for row_idx in range(df_grid.shape[0]):
    for col_idx in range(df_grid.shape[1]):
        val = df_grid.iloc[row_idx, col_idx]
        if pd.notnull(val) and isinstance(val, str) and val.startswith('{'):
            try:
...
user_df = df_flat.groupby('user_id').agg({
    'z_score': 'first',
    'location': 'first',
    'text': 'count' 
}).rename(columns={'text': 'post_count'}).reset_index()

user_df['has_location'] = user_df['location'].apply(lambda x: 1 if pd.notnull(x) and x != "" else 0)
user_df['isBot'] = user_df['user_id'].apply(lambda x: 1 if x in bot_uuids else 0)
user_df['z_score'] = user_df['z_score'].fillna(0)

feature_cols = ['post_count', 'z_score', 'has_location']
...
print(f"\nSample Prediction for user: {'BOT' if prediction[0] == 1 else 'HUMAN'}")